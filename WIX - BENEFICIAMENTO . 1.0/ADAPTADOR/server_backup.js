...
app.post('/api/query/data/find', validateSecretKey, async (req, res) => {
    try {
        ...
        if (sort && Object.keys(sort).length > 0) {
            const sortClauses = Object.entries(sort).map(([field, direction]) => {
                const columnName = getColumnName(field); // Obtém o nome da coluna no banco de dados
                if (direction !== 1 && direction !== -1) {
                    throw new Error(`Direção de ordenação inválida para campo '${field}'. Use 1 (ascendente) ou -1 (descendente).`);
                }
                return `"${columnName}" ${direction === 1 ? 'ASC' : 'DESC'}`;
            });
...
app.post('/api/query/schemas/list', validateSecretKey, async (req, res) => {
    try {
        // Consulta SQL para obter apenas a tabela AD_DIARIOBORDO no Oracle
        const sqlTables = `
            SELECT table_name 
            FROM user_tables 
            WHERE table_name = :tableName
        `;
        const binds = { tableName: 'AD_DIARIOBORDO' }; // Filtra apenas a tabela AD_DIARIOBORDO

        const tablesResult = await executeQuery(sqlTables, binds);

        if (!tablesResult || tablesResult.length === 0) {
            return res.status(404).json({ error: 'Tabela AD_DIARIOBORDO não encontrada no banco de dados' });
        }
