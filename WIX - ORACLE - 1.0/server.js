...
app.post('/api/query/data/find', validateSecretKey, async (req, res) => {
    try {
        ...
        if (filter && Object.keys(filter).length > 0) {
            ...
            if (operator === '$and' && Array.isArray(value)) {
                const conditions = value.map((condition, index) => {
                    const { operator: op, fieldName, value: val } = condition;
                    const bindName = `param_${index}`; // Prefixo seguro para evitar conflitos

                    if (op === '$contains') {
                        binds[bindName] = `%${val}%`; // Adiciona % para LIKE
                        return `"${fieldName}" LIKE :${bindName}`;
                    } else if (op === '$gt') {
                        binds[bindName] = val;
                        return `"${fieldName}" > :${bindName}`;
                    } else {
                        throw new Error(`Operador de filtro '${op}' não suportado.`);
                    }
                }).join(' AND ');

                sql += ` WHERE ${conditions}`;
            } else {
                return res.status(400).json({ error: 'Filtro inválido. Esperado operador $and com um array de condições.' });
            }
        }

        // Adiciona ordenação, se existir
        if (sort && Array.isArray(sort) && sort.length > 0) {
            const sortConditions = sort.map(({ fieldName, direction }) => {
                if (!['asc', 'desc'].includes(direction)) {
                    throw new Error(`Direção de ordenação '${direction}' inválida. Use 'asc' ou 'desc'.`);
                }
        ...
        // Adiciona paginação
        sql += ` OFFSET :param_offset ROWS FETCH NEXT :param_limit ROWS ONLY`;
        binds.param_offset = skip;
        binds.param_limit = limit;
        ...
        // Calcula o total de itens, se necessário
        let totalCount = null;
        if (returnTotalCount) {
...
app.post('/api/query/schemas/list', validateSecretKey, async (req, res) => {
    try {
        // Consulta SQL para obter apenas a tabela AD_WIX no Oracle
        const sqlTables = `
            SELECT table_name 
            FROM user_tables 
            WHERE table_name = :tableName
        `;
        const binds = { tableName: 'AD_WIX' }; // Filtra apenas a tabela AD_WIX

        const tablesResult = await executeQuery(sqlTables, binds);

        if (!tablesResult || tablesResult.length === 0) {
            return res.status(404).json({ error: 'Tabela AD_WIX não encontrada no banco de dados' });
        }
